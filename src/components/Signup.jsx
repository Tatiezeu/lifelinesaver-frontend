...
export default function Signup() {
  ...
  const role = location.state?.role || "emergencyservice";
...
try {
  const res = await axios.post("http://localhost:8080/LifelineJavaBackend/api/authentication/register", {
  ...
  navigate("/signin"); // redirect to signin using router
  } catch (err) {
    if (err.response) {
      ...
      setErrors({ api: "No response from server. Check if GlassFish is running on port 8080." });
    } else {
      setErrors({ api: "Error: " + err.message });
    }
